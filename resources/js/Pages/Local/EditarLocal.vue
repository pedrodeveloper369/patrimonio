...
const form = useForm({
    ...
    tipoLocalizacao: local.value.id_tipolocal,
    local: local.value.caminho_completo,
    id_local: local.value.parent_id,
    estado: local.value.estado,
})

// Função para enviar
const submit = () => {
    form.post(route('local.editar'), {
        onSuccess: () => {
           redirecionar_pagina();
        }
    })
...
<template>
    <AuthenticatedLayout>
        <h4 class="fw-bold py-3 mb-4"><Link :href="route('local')" class="text-muted fw-light">Localizações/</Link><strong>Editar Localização</strong></h4>
