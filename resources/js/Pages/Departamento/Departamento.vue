...
/*
//pegando a lista de departamentos vindo da view
const props = defineProps({
  departamento: Array,
});
const departamentos = ref(props.departamento);
*/
...
//atualiza a lista de departamentos, ao acessar a rota
onMounted(async () => {
  listar_departamentos()
});
//funcao que pega os a lista de departamentos
const listar_departamentos = async () => {
  try {
    const response = await axios.get('/departamento/dados')
    departamentos.value = response.data.data || response.data
  } catch (error) {
    console.error('Erro ao carregar departamentos:', error)
  }
}

//visualização da sigla
// palavras ignoradas
const ignorar = ['de', 'da', 'do', 'dos', 'das', 'e'];

// gera sigla automaticamente para registo
const siglaGerada = computed(() => {
    if (!form.nome) return '';
...
// sincroniza sigla com o formulário
watch(siglaGerada, (novaSigla) => {
    form.sigla = novaSigla;
});

// gera sigla automaticamente para editar
const siglaGerada_editar = computed(() => {
    if (!formEditar.nome) return '';

    return formEditar.nome
        .normalize('NFD')                    // remove acentos
        .replace(/[\u0300-\u036f]/g, '')
        .trim()
        .split(/\s+/)
...
// sincroniza sigla com o formulário
watch(siglaGerada_editar, (novaSigla) => {
    formEditar.sigla = novaSigla;
});


// Função para enviar
const submit = () => {
    form.post(route('departamento.registar'), {
        onSuccess: () => {
            $('#modalRegistar').modal('hide');
            resetModal()
            listar_departamentos()  //recarrega a tabela
...
const submitEliminar = () => {
    formEliminar.post(route('departamento.eliminar'), {
        onSuccess: () => {
...
//editar departamentoutilizador
function editar_departamento(departamento){
    formEditar.reset(); // limpa tudo corretamente

    formEditar.id = departamento.id;
    formEditar.nome = departamento.nome;
    formEditar.sigla = departamento.sigla;
}

// Função para enviar
const submitEditar = () => {
    formEditar.post(route('departamento.editar'), {
        onSuccess: () => {
...
<template>
    <AuthenticatedLayout>
        <h4 class=""><strong>Departamentos</strong></h4>
        ...
        <div class="card p-4 ">
            ...
            <tbody>
                <tr v-for="dep in departamentos" :key="dep.id" :data-id="dep.id">
                <td></td>
                ...
                <td>
                    <button class=""  @click="editar_departamento(dep)"  data-bs-toggle='modal' data-bs-target='#modalEditar'><i class="menu-icon bx bx-edit-alt"></i></button>
                </td>
                </tr>
            </tbody>
            </table>
        ...
        <div class="modal fade" id="modalRegistar" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel3">Registar Departamento</h5>
                        <button
                        type="button"
                        class="btn-close"
                    ...
                    <form @submit.prevent="submit">
                        <div class="modal-body">
                                ...
                                <div class="row" >
                                    ...
                                    <div class="col mb-3">
                                        <label for="nameLarge" class="">Sigla</label>
                                        <input type="text" readonly v-model="form.sigla" class="form-control" placeholder="Enter Name" />
        ...
        <div class="modal fade" id="modalEditar" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel3">Actualizar Departamento</h5>
                        <button
                        type="button"
                        class="btn-close"
                        data-bs-dismiss="modal"
                    ...
                    <form @submit.prevent="submitEditar">
                        <div class="modal-body">
                                ...
                                <div class="row" >
                                    ...
                                    <div class="col mb-3">
                                        <label for="nameLarge" class="">Sigla</label>
                                        <input type="text" readonly v-model="formEditar.sigla" class="form-control" placeholder="Enter Name" />
